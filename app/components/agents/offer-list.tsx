...
export function OfferList({ offers, isLoading, error }: OfferListProps) {
  ...
  const handleOfferClick = (offer: Offer) => {
    router.push(`/agents/create?offer_id=${offer.id}&step=context`);
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            ...
            className="btn-secondary h-10 w-10 flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 transition-colors flex-shrink-0"
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Selecione uma das suas ofertas já criadas para criar o agente a partir delas ou crie um do zero
            </p>
          </div>
        </div>
        <button
