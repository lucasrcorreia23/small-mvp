...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiBody = {
      ...
      offer_description: body.offer_description ?? body.general_description ?? body.description ?? '',
      infer: body.infer ?? false,
    };

    console.log('[STA] Generating offer:', JSON.stringify(apiBody));

    try {
      const response = await fetch(`${ROLE_PLAYS_BASE}/offer/generate`, {
        method: 'POST',
        headers: {
      ...
      console.log('[STA] Generate offer response status:', response.status);
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.error('[STA] Generate offer API error:', data);
        const errMsg = typeof data.detail === 'string' ? data.detail : data.message || data.error || 'Erro ao gerar oferta';
    ...
    console.error('[STA] Erro ao gerar oferta:', error);
    const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
    return NextResponse.json(
      { error: `Erro ao gerar oferta: ${errorMessage}` },
      { status: 500 }
    );
  }
}
