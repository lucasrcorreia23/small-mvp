...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[STA] Creating context:', JSON.stringify(body));

    try {
      const response = await fetch(`${ROLE_PLAYS_BASE}/context/create`, {
        method: 'POST',
        headers: {
      ...
      console.log('[STA] Create context response status:', response.status);
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.error('[STA] Create context API error:', data);
        const errMsg = typeof data.detail === 'string' ? data.detail : data.message || data.error || 'Erro ao criar contexto';
    ...
    console.error('[STA] Erro ao criar contexto:', error);
    const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
    return NextResponse.json(
      { error: `Erro ao criar contexto: ${errorMessage}` },
      { status: 500 }
    );
  }
}
