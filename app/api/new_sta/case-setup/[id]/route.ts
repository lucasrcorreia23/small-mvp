...
export async function GET(
  ...
  try {
    ...
    console.log('[STA] Getting case setup:', id);

    try {
      const response = await fetch(`${ROLE_PLAYS_BASE}/case_setup_${id}`, {
        method: 'GET',
        headers: {
      ...
      console.log('[STA] Get case setup response status:', response.status);
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        console.error('[STA] Get case setup API error:', data);
        const errMsg = typeof data.detail === 'string' ? data.detail : data.message || data.error || 'Erro ao obter case setup';
    ...
    console.error('[STA] Erro ao obter case setup:', error);
    const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido';
    return NextResponse.json(
      { error: `Erro ao obter case setup: ${errorMessage}` },
      { status: 500 }
    );
  }
}
